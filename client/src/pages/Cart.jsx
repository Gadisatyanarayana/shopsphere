  ...
  if (!isAuthenticated) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center space-y-4">
        ...
        <p className="text-xs text-slate-400">You need an active session to view your cart items.</p>
        <Link to="/login" className="inline-block bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold px-6 py-3 rounded-xl">
          Log In Now
        </Link>
      </div>
    );
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center space-y-4">
        ...
        <p className="text-xs text-slate-400 max-w-sm mx-auto">Explore our high performance products and add your favorite items.</p>
        <Link to="/shop" className="inline-block bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold px-6 py-3 rounded-xl">
          Start Shopping
        </Link>
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-6">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Review your selected items before checkout</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {items.map((item) => {
            ...
            return (
              <div key={item._id} className="glass-card p-4 rounded-2xl border border-slate-800 flex items-center gap-4">
                <img
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <Link to={`/product/${product.slug || product._id}`} className="text-sm font-bold text-white hover:text-indigo-400 line-clamp-1">
                    {product.name}
                  </Link>
                  <p className="text-xs text-slate-400">Unit Price: ₹{item.price.toLocaleString('en-IN')}</p>
                </div>
                ...
                <button onClick={() => handleRemove(item._id)} className="text-slate-500 hover:text-rose-400 p-1">
                  <Trash2 size={16} />
                </button>
              </div>
            );
          ...
          <div className="glass-panel p-6 rounded-3xl border border-slate-800 space-y-6">
            ...
            <form onSubmit={handleApplyCoupon} className="space-y-2">
              <label className="text-xs font-semibold text-slate-300 flex items-center gap-1.5">
                <Tag size={14} className="text-indigo-400" /> Apply Coupon Code
              </label>
              <div className="flex gap-2">
                <input
                  ...
                  className="bg-slate-950 border border-slate-800 text-xs font-semibold text-white rounded-xl px-3 py-2.5 outline-none focus:border-indigo-500 uppercase flex-1"
                />
                <button
                  type="submit"
              ...
              {couponError && <p className="text-[11px] font-semibold text-rose-400">{couponError}</p>}
              {appliedCoupon && (
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-extrabold text-xs py-3.5 rounded-2xl shadow-xl shadow-indigo-600/30 transition-all cursor-pointer"
            >
