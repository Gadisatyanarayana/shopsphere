  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900">Seller Product Catalog</h1>
          <p className="text-xs text-slate-500 mt-1">Add new items, update prices, discounts, and control stock availability</p>
        </div>
      ...
      <div className="relative max-w-md">
        <input
          ...
          className="w-full bg-slate-50 border border-slate-200 text-xs text-slate-900 rounded-xl pl-10 pr-4 py-2.5 outline-none focus:border-violet-500 focus:bg-white transition-all"
        />
                ...
                <tr key={prod._id} className="hover:bg-slate-50 transition-colors">
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <Link to={`/product/${prod.slug}`} className="font-bold text-slate-900 hover:text-violet-600 line-clamp-1">
                          {prod.name}
                        </Link>
                  ...
                  <td className="py-4 px-4 font-bold text-slate-500">₹{prod.price.toLocaleString('en-IN')}</td>
